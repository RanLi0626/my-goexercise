...
import (
	...
	mysqlWrapper "goexcersise/award/mysql"
	redisWrapper "goexcersise/award/redis"
)
...
func GetAllAwardBatch() []AwardBatch {
	...
	for index, awardBatch := range awardBatches {
		awardBatches[index].SetTotalAmount(conf.Conf.AwardMap[awardBatch.GetName()])
	}
...
func GetAwardBatch() *AwardBatch {
	...
	// 判断是否到达奖品释放时间点
	startTime, _ := conf.ParseStringToTime(conf.Conf.Award.StartTime)
	endTime, _ := conf.ParseStringToTime(conf.Conf.Award.EndTime)
...
func InitAwardPool() {
	...
	conn.Send("HSET", getAwardBalanceKey(), "A", conf.Conf.AwardMap["A"])
	conn.Send("HSET", getAwardBalanceKey(), "B", conf.Conf.AwardMap["B"])
	conn.Send("HSET", getAwardBalanceKey(), "C", conf.Conf.AwardMap["C"])
	conn.Flush()
